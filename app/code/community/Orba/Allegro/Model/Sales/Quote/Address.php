<?php
class Orba_Allegro_Model_Sales_Quote_Address extends Mage_Sales_Model_Quote_Address
{
    /**
    ...
    public function requestShippingRates(Mage_Sales_Model_Quote_Item_Abstract $item = null)
    {
        ...
        if ($result) {
            ...
            foreach ($shippingRates as $shippingRate) {
				...
				/************************************************************
				 * Start Allegro chnges
				 ************************************************************/
				if($transaction = $this->_getTransaction()){
					if(Mage::getSingleton("orbaallegro/config")->getOverrideRate($this->getQuote()->getStoreId())){
    ...
    protected function _getTransaction()
    {
		if(!$this->hasData("transaction")){
			$quote = $this->getQuote();
			$transactionId = $quote->getOrbaallegroTransactionId();
			$transaction = Mage::getModel("orbaallegro/transaction")->load($transactionId);
			$this->setData("transaction", $transaction->getId() ? $transaction : false);
		}
